...
#pragma region FcLayer
    ...
    FcLayer::FcLayer(int _input_size, int _output_size, int _batch_size): Layer(_input_size, _output_size, _batch_size){
    ...
    void FcLayer::forward(){
            ...
            FcLayerFwd<<<Launch::num_blocks, Launch::threads_per_block, Launch::shared_memory_size>>>(input, output, weights, 
                                                                        biases, weight_counters, bias_counters,
                                                                        input_blocks, output_blocks, batch_blocks,
                                                                        input_bits, output_bits, batch_bits);
            SYNC_KERNEL("FcLayerFwd");

        #else

            throw std::system_error("bmmaBitOpXOR b1 instructions are deprecated for your card.");
    ...
    void FcLayer::back(){
        ...
        not_out_label.load(output_label.host_data, output.bit_dims[0], output.bit_dims[1]);

        Host_Matrix output_label_matrix(output.bit_dims[0], output.bit_dims[1]);
        output_label_matrix.load(output_label.host_data, output.bit_dims[0], output.bit_dims[1]);
        ...
        //fp_error replaces output (which it was set to right above) in the call in order to conform to the LHS/RHS design 
        AND<<<Launch::num_blocks, Launch::threads_per_block>>>(fp_error, not_out_label);
        SYNC_KERNEL("find_fp_error");

        //see previous comment, but with fn_error and not_out
        AND<<<Launch::num_blocks, Launch::threads_per_block>>>(fn_error, output_label_matrix);
        SYNC_KERNEL("find_fn_error");
        
        //fp_error_kernel
        //fn_error_kernel
        //I think this wants to be __CUDACC__, since __CUDA_ARCH__ is only defined on the device and this stuff is on host
        #if  __CUDACC__ >= 800
            ...
            FcLayerBkWeight<<<Launch::num_blocks, Launch::threads_per_block>>>(weights, weight_counters,
                                                                               fp_error_t, fn_error_t,
                                                                               input_t, not_input_t);
...
#pragma region CvLayer
    ...
    // void CvLayer::CvLayer(int _input_size, int _output_size, int _batch_size): Layer(_input_size, _output_size, _batch_size){
