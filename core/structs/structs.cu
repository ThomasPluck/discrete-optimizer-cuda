...
// constructor
Device_Matrix::Device_Matrix(int block_width, int block_height, int bit_width,
                             int bit_height, uchar *_data)
    : Device_Data<uchar>(_data) {
...
__device__ void Device_Matrix::set_chunk(uint eight_x, uint eight_y,
                                         Chunk chunk) {
  uint block_x = eight_x / BLOCK_WIDTH;
  uint block_y = eight_y;
  uint internal_x = eight_x % BLOCK_WIDTH;
...
__device__ void Device_Matrix::set_half_chunks(int eight_x, int eight_y,
                                               uchar *half_1, uchar *half_2) {
  uint block_x = eight_x / BLOCK_WIDTH;
  uint block_y = eight_y;
  uint internal_x = eight_x % BLOCK_WIDTH;
...
//     Transpose<<<Launch::num_blocks,
//     Launch::threads_per_block>>>((*this),(*this).bit_width(),(this*).bit_height());
...
// real constructor
Host_Matrix::Host_Matrix(int bit_width, int bit_height)
    : Host_Data<uchar>(MATRIXSIZE(bit_width, bit_height)) {
...
Host_Matrix::operator Device_Matrix() {
  return Device_Matrix(num_blocks_width(), num_blocks_height(), bit_width(),
                       bit_height(), device_data);
}
