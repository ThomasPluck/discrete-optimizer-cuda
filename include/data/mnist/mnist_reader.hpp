...
bool read_mnist_image_file_flat(Container& images, const std::string& path,
                                std::size_t limit, std::size_t start = 0) {
...
template <template <typename...> class Container = std::vector, typename Image,
          typename Functor>
void read_mnist_image_file(Container<Image>& images, const std::string& path,
                           std::size_t limit, Functor func) {
  auto buffer = read_mnist_file(path, 0x803);
...
void read_mnist_label_file(Container<Label>& labels, const std::string& path,
                           std::size_t limit = 0) {
 ...
 */
template <typename Container>
bool read_mnist_label_file_flat(Container& labels, const std::string& path,
                                std::size_t limit = 0) {
  auto buffer = read_mnist_file(path, 0x801);
...
/*!
 * \brief Read a MNIST label file inside the given flat categorical container
 * (ETL). \param labels The container to fill with the labels \param path The
 * path to the label file \param limit The maximum number of elements to read
 * (0: no limit) \param start The elements to avoid at the beginning
 */
template <typename Container>
...
/*!
 * \brief Read all training images and return a container filled with the
 * images.
 *
...
template <template <typename...> class Container = std::vector, typename Image,
          typename Functor>
Container<Image> read_training_images(const std::string& folder,
                                      std::size_t limit, Functor func) {
...
template <template <typename...> class Container = std::vector, typename Image,
          typename Functor>
Container<Image> read_test_images(const std::string& folder, std::size_t limit,
                                  Functor func) {
  Container<Image> images;
  read_mnist_image_file<Container, Image>(
...
template <template <typename...> class Container = std::vector,
          typename Label = uint8_t>
Container<Label> read_training_labels(const std::string& folder,
                                      std::size_t limit) {
...
template <template <typename...> class Container = std::vector,
          typename Label = uint8_t>
Container<Label> read_test_labels(const std::string& folder,
                                  std::size_t limit) {
  Container<Label> labels;
  read_mnist_label_file<Container, Label>(
 ...
 * The dataset is assumed to be in a mnist subfolder
 *
 * \param training_limit The maximum number of elements to read from training
 * set (0: no limit) \param test_limit The maximum number of elements to read
 * from test set (0: no limit) \return The dataset
 */
template <template <typename...> class Container, typename Image,
          typename Label = uint8_t>
MNIST_dataset<Container, Image, Label> read_dataset_3d(
    const std::string& folder, std::size_t training_limit = 0,
...
MNIST_dataset<Container, Image, Label> read_dataset_3d(
  ...
  return read_dataset_3d<Container, Image, Label>("mnist", training_limit,
                                                  test_limit);
  ;
}
...
MNIST_dataset<Container, Image, Label> read_dataset_direct(
    std::size_t training_limit = 0, std::size_t test_limit = 0) {
  return read_dataset_direct<Container, Image, Label>("mnist", training_limit,
                                                      test_limit);
}
...
MNIST_dataset<Container, Sub<Pixel>, Label> read_dataset(
    const std::string& folder, std::size_t training_limit = 0,
    std::size_t test_limit = 0) {
  return read_dataset_direct<Container, Sub<Pixel>>(folder, training_limit,
                                                    test_limit);
}
