...
#define CUDA_SAFE_CALL(_function_)                                             \
  ...
  if (error != cudaSuccess) {                                                  \
    std::cout << "ERROR: " << cudaGetErrorString(error) << ", " << __FILE__    \
              << ", " << __LINE__ << std::endl;                                \
    exit(EXIT_FAILURE);                                                        \
  }
...
static void allocate_shmem(uint size) { shared_memory_size = size; }

static void print_params() {
  std::cout << std::endl
            << "num blocks: " << num_blocks.x << ", " << num_blocks.y << ", "
            << num_blocks.z << std::endl;
  std::cout << "threads_per_block: " << threads_per_block.x << ", "
            << threads_per_block.y << ", " << threads_per_block.z << std::endl;
  std::cout << "shmem size: " << shared_memory_size << std::endl << std::endl;
}

template <class Func> static void calculate_occupancy(Func kernel) {
  CUDA_SAFE_CALL(cudaGetDeviceProperties(&deviceProp, 0));
  int numBlocksPerSm;
  cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, kernel, 1024,
                                                Launch::shared_memory_size);
  Launch::num_blocks = {numBlocksPerSm * Launch::deviceProp.multiProcessorCount,
                        1, 1};
...
#define CHECK_BOUNDS(_first_, _second_)                                        \
  if ((_FIRST_ >= _first_) || (_SECOND_ >= _second_)) {                        \
    return;                                                                    \
  }